...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<AppState['view']>('auth');
  ...
  const [generatedImageReal, setGeneratedImageReal] = useState<string | null>(null);
  ...
  const handleLogin = async (newUser: User) => {
    ...
    if (loadedProfile) setProfile(loadedProfile);
    else setProfile({ name: newUser.name, heightCm: 175, weightKg: 70, skinTone: SkinTone.Medium, facialFeatures: '', gender: 'Male' });
    setWardrobe(loadedWardrobe);
    setHistory(loadedHistory.sort((a, b) => b.timestamp - a.timestamp));
    setView('profile');
  };
  ...
  const handleUpdateProfile = async (newProfile: UserProfile) => {
    ...
    if (user) await backend.user.updateProfile(user.id, newProfile);
  };

  const handleUpdateWardrobe = async (newWardrobe: WardrobeItem[]) => {
    setWardrobe(newWardrobe);
    if (user) await backend.wardrobe.sync(user.id, newWardrobe);
  };
  ...
  // --- CORE: generate recommendation + background image generation ---
  const handleGenerate = async () => {
    ...
    console.log("handleGenerate: sending", { profile, wardrobe, occasion });

    try {
      const result = await generateOutfit(profile, wardrobe, occasion);
      console.log("handleGenerate: backend result:", result);
      ...
      generateOutfitImage(result, profile, '2D')
        .then((imageResult) => {
          console.log("2D image result:", imageResult);
          setGeneratedImage(imageResult);
          saveToHistory(result, imageResult, null).catch(e => console.warn("saveToHistory failed", e));
        })
        .catch(e => console.warn("2D generation failed", e))
        .finally(() => setIsGenerating2D(false));
    } catch (err: any) {
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <button onClick={toggleTheme} className="p-2 text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-full transition-colors" aria-label="Toggle Dark Mode">
              ...
              {user && (
                <>
                  <button onClick={() => setView('history')} className={`text-sm font-medium transition-colors hidden xs:block sm:block ${view === 'history' ? 'text-primary-600' : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'}`} aria-label="View Style History">History</button>
                  <button onClick={() => setView('history')} className={`sm:hidden p-2 rounded-full ${view === 'history' ? 'text-primary-600 bg-primary-50 dark:bg-primary-900/20' : 'text-neutral-500 dark:text-neutral-400'}`} aria-label="View Style History">
                  ...
                  <button onClick={handleLogout} className="text-xs text-neutral-400 hover:text-red-500 transition-colors whitespace-nowrap" aria-label="Sign out">Sign Out</button>
                </>
              )}
            </div>
          </div>
