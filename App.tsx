...
const App: React.FC = () => {
  ...
  const [generatedImage, setGeneratedImage] = useState<string | null>(null); // 2D / sketch
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        ...
        // Fallback or just show auth if DB fails
        setView('auth');
      } finally {
        setAppReady(true);
      }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // 2. Start Visual (SKETCH) in background
      setIsGenerating2D(true);
      // NOTE: pass "sketch" to backend so server uses illustration prompt
      generateOutfitImage(result, profile, 'sketch')
        .then((imageResult) => {
  ...
  const handleGenerateReal = async () => {
      ...
      try {
        ...
        // NOTE: pass "real" to backend so server uses photorealistic prompt
